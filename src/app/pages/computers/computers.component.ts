  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="card-smp p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <input [(ngModel)]="search" (ngModelChange)="onFilterChange()" class="input-smp" placeholder="Buscar por ID, código u observación" />
        <select [(ngModel)]="selectedStatus" (ngModelChange)="onFilterChange()" class="select-smp"><option value="">Todos los estados</option><option value="Bueno">Bueno</option><option value="Regular">Regular</option><option value="Malo">Malo</option><option value="Descarte">Descarte</option></select>
      </div>
